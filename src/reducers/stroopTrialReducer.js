...
export default function stroopTrialReducer(state, action) {
  ...
  switch (type) {
    ...
    case "KEY_PRESS":
      const { mode, roundNum, keyPressed, isCorrect, isCongruent, pressTime } =
        payload;
    ...
    case "PUSH_TIME_START": {
      const { mode, roundNum, h, m, ms, sec } = payload;
      let newList = state.startTimeList;
      newList.push({ roundNum, mode, h, m, sec, ms });
      return {
        ...state,
    ...
    case "PUSH_TIME_END": {
      const { mode, roundNum, h, m, ms, sec, keyDown } = payload;
      let newList = state.endTimeList;
      newList.push({ roundNum, mode, h, m, sec, ms, keyDown });
      return {
        ...state,
        endTimeList: newList,
      };
